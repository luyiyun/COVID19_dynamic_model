...
from SEAIRQ import SEAIRQ, score_func
...
if __name__ == "__main__":
    ...
    parser.add_argument("--t0", default="2020-01-23", help="疫情开始的那天")
    ...
    # 先一个省份看看
    reg = "山东"
    region_index = dats["regions"].index(reg)
    ...
    epi_t0_relative = dats["epidemic_t0"] - t0  # 数据时间
    ...
    if args.model is not None and args.model != "fit":
        model = SEAIRQ.load(args.model)
    else:
        model = SEAIRQ(
            populations=dats["population"][region_index], t0=t0,
            y0for1=[0, 0, 0, 0, 0, 0, 0, 0], protect=True,
            protect_args={"t0": protect_t0_relative[region_index],
                          "c_k": 0.0, "q_k": 0.0},
        ...
        if args.model == "fit":
            ...
            use_fit_data_epid = {
                ...
                "trueH": dats["trueH"][use_fit_data_start:, region_index],
                "trueR": dats["trueR"][use_fit_data_start:, region_index],
                "trueD": dats["trueD"][use_fit_data_start:, region_index],
            }

            # 搜索
            def func(x):
                return score_func(x, model, **use_fit_data_epid)
            ...
            #     func, np.stack([lb, ub], axis=1), maxiter=1000,
    ...
    true_area = trapz(epi_times_relative, dats["trueH"][:, region_index])
    pred_area = trapz(epi_times_relative, predH_nopr_only_true)
    diff_areas = pred_area - true_area
    # results["area"] = {
