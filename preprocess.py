...
def preprocess_provinces_pmn():
    ...
    # 得到所有流量数据
    prov_files = [f for f in os.listdir(FILES["Prov"]["pmn_dir"])
                  if f.endswith("csv")]
    ...
    for prov_name in prov_names:
        dat = pd.read_csv(
            os.path.join(FILES["Prov"]["pmn_dir"], "%s.csv" % prov_name),
            index_col=0
        )
    ...
    # 整理时间点
    all_times = list(set(all_provs.index))
    print("一共有%d个时间点。" % len(all_times))
    common_list = list(common)
...
def preprocess_provinces_epidemic():
    # 读取数据，并做初步的处理
    epidemic_dat = pd.read_csv(FILES["Prov"]["epidemic_file"])
...
def preprocess_provinces_population():
    # 读取数据
    population_dat = pd.read_csv(FILES["Prov"]["population_file"])
    population_dat.rename(
...
def preprocesss_provinces_first_time():
    ...
    def parse_time(t):
        ...
        return parse(t).toordinal()

    response_time = pd.read_csv(
        FILES["Prov"]["response_time_file"], header=None).iloc[:, :2]
    ...
    first_case = pd.read_csv(FILES["Prov"]["first_case_file"], header=None)
...
def preprocess_provinces_outtrend():
    df = pd.read_csv(FILES["Prov"]["out_trend_file"])
    df = df.iloc[:, 1:]
